  ...
  return (
    <ContentWrapper
      ...
      className={`group relative w-full block py-12 lg:py-16 border-b border-zinc-700 flex flex-col lg:flex-row items-start lg:items-center gap-6 lg:gap-0 overflow-hidden px-4 sm:px-6 lg:px-10 transition-colors duration-500 ease-out px-5 lg:px-10 ${
        ...
        isActive ? 'bg-[#FF5733]' : 'bg-transparent'
      }`}
    >
      ...
      <div className={`w-full lg:w-2/12 flex items-center transition-all duration-500 ${isActive ? 'lg:translate-x-4 text-black' : 'text-zinc-600'}`}>
      ...
      <div className="w-full lg:w-5/12">
        <h3 className={`text-3xl lg:text-4xl xl:text-5xl font-medium transition-all duration-500 ease-out ${isActive ? 'text-black lg:translate-x-4' : 'text-zinc-300'}`}>
          {project.name}
        </h3>
      </div>
      ...
      <div className="w-full lg:w-3/12 pr-0 lg:pr-10">
        <p className={`text-sm lg:text-base leading-relaxed transition-all duration-500 ${isActive ? 'text-black/80 lg:-translate-x-2' : 'text-zinc-500'}`}>
          {project.description}
        </p>
      </div>
      ...
      <div className={`w-full lg:w-2/12 flex items-center lg:justify-end gap-2 transition-all duration-500 ${isActive ? 'lg:-translate-x-4' : ''}`}>
        {project.isOngoing ? (
          <span className={`text-[0.65rem] uppercase tracking-[0.2em] font-bold transition-colors duration-500 ${isActive ? 'text-black' : 'text-zinc-600'}`}>
            Ongoing
          </span>
        ) : (
          ...
          <div className={`flex items-center justify-center w-12 h-12 rounded-full border transition-colors duration-500 ${
            isActive ? 'bg-black border-black text-[#f3f3f3]' : 'bg-transparent border-zinc-600 text-zinc-600'
          }`}>
