...
const LabItem = ({ project, index, registerItem }) => {
  ...
  return (
    <div ref={triggerRef} className="w-full relative">
      <ContentWrapper
        {...wrapperProps}
        onPointerEnter={(e) => { if (window.innerWidth >= 1024) setActiveProject(index); }}
        onPointerLeave={(e) => { if (window.innerWidth >= 1024) setActiveProject(null); }}
      ...
      >
        ...
        <div className="relative w-full aspect-square mb-4 lg:mb-6">
          ...
          <div ref={containerRef} className="absolute inset-0 bg-[#0a0a0a] overflow-hidden rounded-sm z-0">
        ...
        <div className="w-full flex flex-col sm:flex-row sm:items-center justify-between border-t border-zinc-700 pt-4 gap-3 sm:gap-0 relative z-20">
          <div className="flex items-center gap-3 lg:gap-4">
            <span className="text-xs font-bold tracking-[0.2em] text-zinc-600">0{index + 1}</span>
          ...
          <div className="flex flex-wrap gap-x-2 lg:gap-x-3 gap-y-1">
            {project.tools.map((tag, i) => (
              <React.Fragment key={tag}>
                <span className="text-[0.6rem] lg:text-[0.65rem] uppercase tracking-[0.2em] font-bold text-zinc-500 lg:group-hover:text-[#f3f3f3] group-[.is-active]:text-[#f3f3f3] transition-colors duration-500">{tag}</span>
                {i !== project.tools.length - 1 && <span className="text-zinc-800">/</span>}
              </React.Fragment>
            ))}
          </div>
        </div>
